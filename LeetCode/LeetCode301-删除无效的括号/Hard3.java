...
public class Hard3 {
	private Set<String> set = new HashSet<>();	//集合用于存答案
	public List<String> removeInvalidParentheses(String s) {
		char[] ss = s.toCharArray();
		int open = 0,close = 0;
		for (char c : ss) {		//统计多余的左右括号数量
            if (c == '(') 
            	open ++;
            else if (c == ')') {
            	 if (open > 0) 	//括号匹配则不需要去除
                 	open --;
            	 else 
                 	close ++;
            }      
	...
	public void backTracking(char[] ss, StringBuilder sb, 
			int index, int open, int close, int openRem, int closeRem) {
		/**   
